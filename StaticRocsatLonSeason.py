...
def static_EPB2():
    ...
    for i in range(12):
        for j in range(72):
            print(abin2[i][j])
    fill_PBoccurence2(abin2, outfilename)
...
def draw_V_PB_BD_Ne():
    ...
    draw_one_panel('lgNe_1999_2004.txt')

    plt.subplot(2, 2, 2)
    plt.title('ROCSAT PB')
    draw_one_panel('EPB_1999_2004_a.txt')
    ...
    plt.subplot(2, 2, 4)
    plt.title('ROCSAT Vz')
    draw_one_panel('V_drift_1999_2002.txt')
...
def draw_one_panel(filename):
    ...
    plt.imshow(result2, cmap='gnuplot')
    plt.colorbar()
    plt.yticks(np.arange(0, 12), np.arange(1, 13))
    plt.xticks(np.arange(0, 25, 2) - 0.5, np.arange(-180, 181, 30))
    plt.axis([-0.5, 23.5, -0.5, 11.5])
